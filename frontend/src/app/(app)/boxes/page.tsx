...
type ShelfItem = { idShelf: number; archive_id: number; shelf_code: string; shelf_name: string };
    ...
    <>
      ...
      <div className="split">
        <section className="card"><h2>Nueva caja</h2><form className="form-grid" onSubmit={submit}>
          ...
          <label>Estanteria<select value={shelfId} onChange={(event) => setShelfId(event.target.value)}><option value="">Sin estanteria</option>{filteredShelves.map((item) => <option key={item.idShelf} value={item.idShelf}>{item.shelf_code} - {item.shelf_name}</option>)}</select></label>
      ...
      <DetailDrawer open={Boolean(detail)} title={detail?.box_code ?? "Caja"} subtitle={detail?.location_path} onClose={() => setDetail(null)}>
        {detail ? <section className="grid">
          ...
          <div className="card compact"><h3>Mover caja</h3><div className="toolbar"><select value={moveShelfId} onChange={(event) => setMoveShelfId(event.target.value)}><option value="">Seleccionar estanteria</option>{filteredShelves.map((item) => <option key={item.idShelf} value={item.idShelf}>{item.shelf_code} - {item.shelf_name}</option>)}</select><button disabled={!moveShelfId || move.isPending} onClick={() => move.mutate()}><MoveRight size={16} /> Mover</button></div><p className="muted">Si el destino pertenece a otro archivo, AMBAR lo bloquea y pide transferencia documental.</p></div>
