...
type ShelfNode = { idShelf: number; shelf_code: string; shelf_name: string; aisle?: string | null; floor?: string | null; module?: string | null; bay?: string | null; body?: string | null; level?: string | null; topographic_path?: string | null; current_boxes: number; capacity_boxes: number; occupancy_percent: number; status: string; boxes: BoxNode[] };
...
function shelfPath(shelf: ShelfNode) {
  return shelf.topographic_path || [shelf.aisle ? `Pasillo ${shelf.aisle}` : null, `Estanteria ${shelf.shelf_code}`, shelf.body || shelf.module ? `Cuerpo ${shelf.body ?? shelf.module}` : null, shelf.level || shelf.bay ? `Nivel ${shelf.level ?? shelf.bay}` : null].filter(Boolean).join(" / ");
}
