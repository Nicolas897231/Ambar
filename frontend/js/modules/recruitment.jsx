...
function VacancyModal({ onClose, onCreated }) {
  ...
  const submit = async () => {
    const missing = [];
    if (!payload.vacancy_code.trim()) missing.push("codigo");
  ...
  return (
    <Modal title="Nueva vacante" sub="Se publica en el pipeline interno y en el portal publico si queda abierta." onClose={onClose}
      ...
      <div className="grid cols-2" style={{ gap: "var(--s4)" }}>
        <Field label="Codigo" required><input value={payload.vacancy_code} onChange={(e) => setField("vacancy_code", e.target.value)} /></Field>
        ...
        <Field label="Ubicacion"><input value={payload.location} onChange={(e) => setField("location", e.target.value)} placeholder="Cali, Bogota, remoto..." /></Field>
        <div style={{ gridColumn: "1 / -1" }}><Field label="Descripcion"><textarea value={payload.description} onChange={(e) => setField("description", e.target.value)} /></Field></div>
        <div style={{ gridColumn: "1 / -1" }}><Field label="Requisitos" help="Uno por linea"><textarea value={payload.requirements} onChange={(e) => setField("requirements", e.target.value)} placeholder={"Experiencia en archivo\nManejo de Excel\nAtencion al detalle"} /></Field></div>
      </div>
    </Modal>
  );
}
    ...
    <>
      <div className="page-head">
        ...
        <div className="page-actions">
          <Button variant="ghost" icon="external-link" onClick={() => navigate("empleo")}>Ver portal publico</Button>
          {can(user, ["recruit.manage"]) && <Button icon="plus" onClick={() => setCreating(true)}>Nueva vacante</Button>}
        </div>
      </div>
      ...
      {tab === "vacancies" && (
        <div className="grid cols-2 stagger">
          ...
          {vacancies.map((v, i) => (
            <Card key={v.idVacancy || v.id || i} interactive style={{ "--i": i }} className="vacancy-card">
              <div className="row between">
                ...
                <Badge tone={v.status === "open" || v.status === "active" || v.status === "Activa" ? "success" : "warning"} dot>{v.status || "active"}</Badge>
              </div>
            </Card>
          ))}
        </div>
