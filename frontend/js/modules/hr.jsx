...
function EmployeeProfile({ emp, onClose, navigate }) {
  ...
  const tabs = [{ key: "info", label: "Informacion", icon: "user" }, { key: "contracts", label: "Contratos", icon: "file-text" }, { key: "expedient", label: "Expediente", icon: "folder-kanban" }, { key: "medical", label: "Examenes medicos", icon: "stethoscope" }, { key: "novelty", label: "Novedades", icon: "flag" }];
  return (
    <Drawer wide title={emp.name} sub={emp.pos} onClose={onClose} headExtra={<Badge tone={EMP_STATE[emp.state] || "neutral"} dot>{emp.state}</Badge>}>
      ...
      {tab === "contracts" && <Empty icon="file-text" title="Contratos no cargados">Los contratos se listan cuando existan registros reales asociados al empleado.</Empty>}
...
function PositionModal({ onClose, onCreated, departments }) {
  const toast = useToast();
  const [payload, setPayload] = hrS({ position_code: "", name: "", level: "operativo", department: "", description: "" });
  const setField = (key, value) => setPayload(p => ({ ...p, [key]: value }));
  const submit = async () => {
    if (!payload.position_code.trim() || !payload.name.trim() || !payload.department.trim()) {
      toast("Codigo, nombre y dependencia son obligatorios.", { tone: "danger", title: "Cargo incompleto" });
      return;
    }
    try {
      const created = await AmbarAPI.post("/hr/positions", payload);
  ...
  return (
    <Modal title="Nuevo perfil de cargo" sub="Los cargos alimentan usuarios, empleados y checklist documental." onClose={onClose}
      ...
      <div className="grid cols-2" style={{ gap: "var(--s4)" }}>
        <Field label="Codigo" required><input maxLength={40} value={payload.position_code} onChange={e => setField("position_code", e.target.value)} placeholder="CAR-001" /></Field>
        ...
        <Field label="Dependencia" required><select value={payload.department} onChange={e => setField("department", e.target.value)}><option value="">Seleccionar dependencia</option>{departments.map(d => <option key={d.idDepartment || d.id || d.name} value={d.name || d.department_name}>{d.name || d.department_name}</option>)}</select></Field>
        <div style={{ gridColumn: "1 / -1" }}><Field label="Descripcion"><textarea maxLength={500} value={payload.description} onChange={e => setField("description", e.target.value)} /></Field></div>
      </div>
    </Modal>
  );
}
...
function DepartmentModal({ onClose, onCreated }) {
  ...
  return (
    <Modal title="Nueva dependencia" sub="La dependencia organiza cargos y expedientes laborales." onClose={onClose}
      ...
      <div className="grid cols-2" style={{ gap: "var(--s4)" }}>
        <Field label="Codigo" required><input maxLength={40} value={payload.department_code} onChange={e => setField("department_code", e.target.value)} placeholder="RRHH" /></Field>
        ...
        <Field label="Responsable"><input inputMode="numeric" maxLength={12} value={payload.responsible_identification} onChange={e => setField("responsible_identification", e.target.value.replace(/\D/g, ""))} placeholder="Identificacion" /></Field>
      </div>
    </Modal>
  );
}
...
function HRPage({ user, navigate }) {
  ...
  return (
    <>
      <div className="page-head"><div><div className="eyebrow">Talento Humano</div><h1>Recursos Humanos</h1><p className="lead">Administra el ciclo de vida de tus colaboradores: datos, contratos, expediente documental, examenes medicos y novedades, todo conectado al archivo.</p></div><div className="page-actions">{canManage && <><Button variant="ghost" icon="download" onClick={exportRows}>Exportar</Button><Button icon="user-plus" onClick={() => setModal("employee")}>Nuevo empleado</Button></>}</div></div>
      ...
      {tab === "employees" && (
        <Card flush className="an-rise">
          ...
          <div className="table-scroll"><table className="tbl"><thead><tr><th>Empleado</th><th>Cargo</th><th>Area</th><th>Contrato</th><th>Ingreso</th><th>Expediente</th><th>Estado</th><th></th></tr></thead><tbody>
      ...
      {tab === "contracts" && (
        <Card flush className="an-rise"><div className="table-scroll"><table className="tbl"><thead><tr><th>Empleado</th><th>Tipo</th><th>Inicio</th><th>Fin</th><th>Estado</th></tr></thead><tbody>
          ...
          {liveContracts.data.length === 0 && <tr><td colSpan="5"><Empty icon="file-clock" title="Sin contratos por vencer">No hay contratos proximos a vencer en la base de datos.</Empty></td></tr>}
