...
function DocumentsPage({ user, navigate }) {
  ...
  const liveDocs = window.useLiveData(
    () => window.AmbarAPI.endpoints.documents().then(items => items.map((item, i) => ({
      id: item.document_code || item.idDocument || `DOC-${i + 1}`,
      name: item.title || item.document_name || item.name || item.document_code || "Documento sin título",
      type: item.type_name || item.type_documental || item.document_type || "Documento",
      area: item.dependency_name || item.area || item.archive_name || "Archivo",
      resp: item.created_by_name || item.responsible_name || item.created_by || "AMBAR",
      state: item.status || "Activo",
      date: item.created_at ? String(item.created_at).slice(0, 10) : "—",
      due: item.due_date ? String(item.due_date).slice(0, 10) : "—",
      folios: item.total_folios || item.folio_count || 0,
  ...
  const rows = docM(() => liveDocs.data.filter(d =>
    (!q || (d.name + d.id + d.resp).toLowerCase().includes(q.toLowerCase())) &&
    (!fState || d.state === fState) && (!fArea || d.area === fArea)), [q, fState, fArea, liveDocs.data]);
