...
function EmployeeProfile({ emp, onClose, navigate }) {
  ...
  return (
    <Drawer wide title={emp.name} sub={emp.pos} onClose={onClose} headExtra={<Badge tone={EMP_STATE[emp.state]} dot>{emp.state}</Badge>}>
      ...
      {tab === "contracts" && <Empty icon="file-text" title="Contratos no cargados">Los contratos deben consultarse desde el endpoint de contratos laborales cuando exista información para este empleado.</Empty>}
      {tab === "expedient" && (<><div className="page-intro" style={{ marginBottom: "var(--s4)" }}><span className="pi-ico"><Icon name="folder-kanban" size={18} /></span><div><h4>Expediente laboral vinculado</h4><p>Abre el módulo documental para ver documentos reales asociados al expediente.</p></div></div><Button variant="ghost" className="btn-block" icon="folder-kanban" onClick={() => navigate && navigate("expedients")}>Abrir expedientes</Button></>)}
      {tab === "medical" && (<div className="col gap2"><Empty icon="stethoscope" title="Sin exámenes cargados">No se muestran exámenes ficticios. Consulta SST para registros reales.</Empty><Button variant="ghost" className="btn-block" icon="stethoscope" onClick={() => navigate && navigate("medical")}>Ver módulo SST</Button></div>)}
  ...
  return (
    <>
      ...
      <div className="grid cols-4 stagger">
        ...
        <Metric label="Contratos por vencer" value={liveContracts.data.length} icon="file-clock" tone="danger" accent foot="próximos 30 dias" />
