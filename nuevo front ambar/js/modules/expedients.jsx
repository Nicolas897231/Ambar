...
function ExpedientsPage({ user, navigate }) {
  ...
  const rows = EXPEDIENTS.filter(e => (!type || e.type === type) && (!q || (e.name + e.id).toLowerCase().includes(q.toLowerCase())));
  ...
  return (
    <>
      ...
      <div className="grid" style={{ gridTemplateColumns: "repeat(6, 1fr)" }}>
        {EXP_TYPES.map((t, i) => {
          const count = EXPEDIENTS.filter(e => e.type === t.key).length;
      ...
      <Card flush className="an-rise">
        ...
        <div className="table-scroll">
          <table className="tbl">
            ...
            <tbody>
              {rows.map(e => {
                const tc = EXP_TYPES.find(t => t.key === e.type);
